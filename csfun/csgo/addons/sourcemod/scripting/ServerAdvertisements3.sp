...
public Action Command_sa3(int client, int args)
{
    if(IsValidClient(client))
    {
        ...
        {
            ArrayList aRtemp = view_as<ArrayList>(CloneHandle(aMessagesList.Get(i)));
            ...
            if(aRtemp)
            {
                ...
                ArrayList aRtempText = view_as<ArrayList>(CloneHandle(aRtemp.Get(4)));
...
{
    // Delete all ArrayList's in aMessagesList
    for(int i = 0; i < aMessagesList.Length; i++)
    {
...
{
    ArrayList aRtemp = view_as<ArrayList>(CloneHandle(aMessagesList.Get(g_iCurrentMessage)));
    
    if (aRtemp != null && aRtemp.Length > 0)
    {
        char sType[32];
        char sTag[64];
        char sFlags[16];
